...
pub async fn execute(config_path: Option<&str>, verbose: bool, force_download: bool, update_config: bool) -> Result<()> {
    let mut cmd = BuildCommand::new();
    cmd.execute(config_path.map(|s| s.to_string()), verbose, force_download, update_config).await
}
...
impl BuildCommand {
    ...
    pub async fn execute(&mut self, config_path: Option<String>, verbose: bool, force_download: bool, update_config: bool) -> Result<()> {
        ...
        log::info!("Starting build process for entry file: {}", config.build.entry_file.display());
    ...
    async fn load_build_config(&self, config_path: Option<String>) -> Result<BuildConfig> {
        ...
        if !Path::new(&config_file).exists() {
            return Err(OpenCliError::NotFound(format!(
    ...
    async fn compile_project(&self, config: &BuildConfig, compiler_path: &Path, verbose: bool) -> Result<()> {
        ...
        if !entry_path.exists() {
            return Err(OpenCliError::NotFound(format!("Entry file not found: {}", entry_path.display()).into()));
        }
        ...
        if verbose || has_debug_flags {
            ...
            let mut child = cmd.spawn()
                .map_err(|e| OpenCliError::Process(format!("Failed to execute compiler: {}", e).into()))?;
            
            let status = child.wait().await
                .map_err(|e| OpenCliError::Process(format!("Failed to wait for compiler: {}", e).into()))?;
            
            if status.success() {
                let compile_duration = compile_start.elapsed();
                let time_str = format_duration(compile_duration);
                println!("Build successful: {} ({})", config.build.output_file.display(), time_str);
                log::info!("Build completed successfully: {} in {}", config.build.output_file.display(), time_str);
            } else {
                return Err(OpenCliError::Process(format!("Build failed with exit code: {}", 
