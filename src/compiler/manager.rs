...
impl CompilerManager {
    ...
    async fn download_compilers_config(config_path: &Path) -> Result<CompilerConfig> {
        const COMPILERS_CONFIG_URL: &str = "https://gist.githubusercontent.com/mateuphinxx/594288e39043b144d44364cbf3233b10/raw/compilers.toml";
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| OpenCliError::Process(format!("Failed to download compilers config: {}", e).into()))?;
        
        if !response.status().is_success() {
            return Err(OpenCliError::Process(format!("Failed to download compilers config: HTTP {}", response.status()).into()));
        }
        
        let content = response.text().await
            .map_err(|e| OpenCliError::Process(format!("Failed to read compilers config: {}", e).into()))?;
        ...
        let config: CompilerConfig = toml::from_str(&content)
            .map_err(|e| OpenCliError::Config(format!("Invalid compilers config format: {}", e).into()))?;
        
        Ok(config)
    }
    
    pub async fn get_compiler_path(&mut self, version: &str, force_download: bool) -> Result<PathBuf> {
        ...
        self.download_and_install_compiler(version, platform_config).await?;
        ...
        if binary_path.exists() {
            ...
            self.cache.store_hash(&platform_config.binary, &file_hash).await?;
            ...
            Err(OpenCliError::NotFound(format!("Compiler binary not found after installation: {}", binary_path.display()).into()))
        }
    }
    
    async fn download_and_install_compiler(&self, version: &str, platform_config: &PlatformConfig) -> Result<()> {
        ...
        let matching_asset = self.downloader.find_matching_asset(&assets, &platform_config.match_pattern).await?;
        ...
        self.downloader.download_asset(matching_asset, &downloaded_file).await?;
        ...
        match platform_config.method.as_str() {
            ...
            _ => return Err(OpenCliError::Config(format!("Unsupported extraction method: {}", platform_config.method).into())),
        }
    ...
    async fn extract_zip(&self, archive_path: &Path, extract_to: &Path) -> Result<()> {
        ...
        let mut archive = zip::ZipArchive::new(file)
            .map_err(|e| OpenCliError::Process(format!("Failed to read zip archive: {}", e).into()))?;
        
        for i in 0..archive.len() {
            let mut file = archive.by_index(i)
                .map_err(|e| OpenCliError::Process(format!("Failed to read zip entry: {}", e).into()))?;
    ...
    async fn extract_tgz(&self, archive_path: &Path, extract_to: &Path) -> Result<()> {
        ...
        archive.unpack(extract_to)
            .map_err(|e| OpenCliError::Process(format!("Failed to extract tar.gz: {}", e).into()))?;
        
        Ok(())
    }
    
    async fn organize_files(&self, extract_dir: &Path, platform_config: &PlatformConfig) -> Result<()> {
        for (pattern, target) in &platform_config.paths {
            let regex = Regex::new(pattern)
                .map_err(|e| OpenCliError::Config(format!("Invalid path pattern: {}", e).into()))?;
            
            self.search_and_move_recursively(extract_dir, &regex, target).await?;
        }
    ...
    async fn search_and_move_recursively(&self, dir: &Path, regex: &Regex, target: &str) -> Result<bool> {
        fn visit_dir(dir: &Path, regex: &Regex, target: &str, base_dir: &Path) -> std::result::Result<bool, std::io::Error> {
            for entry in std::fs::read_dir(dir)? {
                ...
                } else if let Some(relative_path) = path.strip_prefix(base_dir).ok() {
    ...
    async fn cleanup_empty_dirs(&self, dir: &Path) -> Result<()> {
        ...
        remove_empty_dirs_recursive(dir)
            .map_err(|e| OpenCliError::Process(format!("Failed to cleanup directories: {}", e).into()))?;
